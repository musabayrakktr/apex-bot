...
# --- SAKİN VE AGRESİF MOD SİSTEMİ (YENİ EKLENEN KISIM) ---
current_mode = "agresif"  # Başlangıç modu: Agresif (Avcı)

MODS = {
    "sakin": {
        "interval": 60,       # Sakin modda tarama aralığı: 60 saniye
        "rsi_threshold": 60,  # Sakin mod RSI eşiği
        "take_profit": 0.015, # %1.5 Kâr Al
        "stop_loss": 0.02,    # %2.0 Zarar Kes
        "name": "🐢 Sakin Mod"
    },
    "agresif": {
        "interval": 20,       # Agresif modda tarama aralığı: 20 saniye
        "rsi_threshold": 65,  # Agresif mod RSI eşiği
        "take_profit": 0.018, # %1.8 Kâr Al
        "stop_loss": 0.025,   # %2.5 Zarar Kes
        "name": "⚡ Agresif Mod (Avcı)"
    }
}
# ---------------------------------------------------------
...
# TELEGRAM KOMUTLARINI DİNLEME VE MOD DEĞİŞTİRME FONKSİYONU
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if "result" in res and res["result"]:
                for update in res["result"]:
                    ...
                    if text == "/sakin" and current_mode != "sakin":
                        current_mode = "sakin"
                        send_telegram("⚙️ *Sistem Sakin Moda alındı kanka.* 60 saniyede bir taranıyor, risk minimumda.")
                    elif text == "/agresif" and current_mode != "agresif":
                        current_mode = "agresif"
                        send_telegram("⚡ *Sistem Agresif Moda fırladı kanka!* 20 saniyede bir tarama aktif, avcı moddayız.")
    except Exception as e:
        pass
...
def set_telegram_commands():
    ...
    commands = [
        ...
        {"command": "sakin", "description": "🐢 Botu Sakin Moda Al (60s tarama)"},
        {"command": "agresif", "description": "⚡ Botu Agresif Moda Al (20s tarama)"},
...
def check_auto_trade_signals():
    ...
    # Aktif moda göre dinamik eşikleri alıyoruz
    aktif_ayar = MODS[current_mode]
    target_rsi = aktif_ayar["rsi_threshold"]
    tp_oran = aktif_ayar["take_profit"]
    sl_oran = aktif_ayar["stop_loss"]
    ...
    for coin in coins:
        ...
        is_strong_dip = (rsi <= (target_rsi - 32)) or (rsi <= target_rsi and bb_l > 0 and curr_p <= bb_l * 1.002)
        ...
        if is_strong_dip and last_trade_state[coin] != "BOUGHT":
            ...
            if trade_amount >= 5.0:
                ...
                if success:
                    ...
                    send_telegram(
                        f"🚨 *[İŞLEM BİLDİRİMİ: ALIM YAPILDI ({MODS[current_mode]['name']})]*\n"
                        f"━━━━━━━━━━━━━━━━━━━\n"
                        f"🪙 **Coin:** `{coin.upper()}`\n"
                        f"💵 **Alış Fiyatı:** `{curr_p:,.2f}` $\n"
                        f"💰 **Kullanılan Tutar:** `{trade_amount}` USDT\n"
                        f"📊 **Sinyal:** RSI `{rsi}` + Bollinger Desteği\n"
                        f"🛡️ **Stop-Loss:** `%{sl_oran*100:.1f}` | 🎯 **Hedef Kâr:** `%{tp_oran*100:.1f}`\n"
                        f"━━━━━━━━━━━━━━━━━━━",
            ...
            elif pnl_pct >= tp_oran and not partial_tp_done[coin]:
...
def background_worker():
    ...
    while True:
        try:
            # 1. Önce Telegram'dan /sakin veya /agresif komutu gelmiş mi diye kontrol et
            komutlari_kontrol_et()
            
            # 2. Canlı verileri çek
            fetch_live_data()
            
            # 3. Otomatik al-sat sinyallerini kontrol et
            check_auto_trade_signals()
            
        except Exception as e:
            print(f"Arka plan worker hatası: {e}")
            
        # 4. Aktif modun süresine göre dinamik bekle (Sakin: 60s, Agresif: 20s)
        aktif_sure = MODS[current_mode]["interval"]
        time.sleep(aktif_sure)
...
def index():
    ...
    return render_template_string(DASHBOARD_TEMPLATE, cache=crypto_cache, mode=MODS[current_mode]["name"])
