...
def get_live_market_data():
    """Tüm piyasa ve kur verilerini engelsiz ve canlı kaynaklardan çeker"""
    global _cache
    now = time.time()
    
    # 5 dakika (300 saniye) boyunca cache'den hızlı servis eder
    ...
    # 2. Döviz Kuru (En kararlı açık kaynak JSON uç noktası)
    usd_try = 34.20
    try:
        ...
        print(f"Dolar Hata: {e}")

    # 3. Canlı Altın Hesaplama (Serbest piyasa güncel ons bazlı)
    try:
        # Yaklaşık güncel ons altın baz alınarak kur ile çarpılır
        gram_tln = (2650.0 / 31.1035) * usd_try
        gram_altin_str = f"{gram_tln:,.2f} TL"
        ceyrek_altin_str = f"{(gram_tln * 1.75):,.2f} TL"
    except Exception as e:
        print(f"Altın Hata: {e}")
